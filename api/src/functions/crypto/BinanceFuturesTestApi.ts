...
export async function createPosition(symbol: string, side: string, quantity: number,  secretKey: string, ApiKey: string) {
  ...
  const finalUrl = `${API_URL_TEST}fapi/v1/order?${totalParams}&signature=${signature}`;
...
export async function getPrice(symbol: string, apiKey: string) {
  const finalUrl = `${API_URL_TEST}fapi/v1/premiumIndex?symbol=${symbol}`;
  const option = {
    method: "GET",
    headers: {
      'X-MBX-APIKEY': apiKey
...
export async function getPosition(symbol: string, apiKey: string, secretKey: string) {
  ...
  const signature = HmacSHA256(totalParams, secretKey);
  const finalUrl = `${API_URL_TEST}fapi/v2/positionRisk?${totalParams}&signature=${signature}`;
  const option = {
    method: "GET",
    headers: {
      'X-MBX-APIKEY': apiKey
...
export async function getBalance(apiKey: string, secretKey: string) {
  ...
  const signature = HmacSHA256(totalParams, secretKey);
  const finalUrl = `${API_URL_TEST}fapi/v2/balance?${totalParams}&signature=${signature}`;
  const option = {
    method: "GET",
    headers: {
      'X-MBX-APIKEY': apiKey
